...
typedef enum evolis_model_e {
    ...
    EVOLIS_MO_BADGEPASS_BADGY100   = 0x00008000,
    EVOLIS_MO_BADGEPASS_BADGY200   = 0x00010000,
    EVOLIS_MO_EDIKIO_ACCESS        = 0x00020000,
    EVOLIS_MO_EDIKIO_FLEX          = 0x00040000,
    EVOLIS_MO_EDIKIO_DUPLEX        = 0x00080000,
} evolis_model_t;
...
/// return:The size of the read data or -1 on error
...
typedef struct evolis_infos_s {
    ...
    bool            hasEthernet;                //!< ``1`` if the printer have ethernet feature, ``0`` otherwise.
    bool            hasWifi;                    //!< ``1`` if the printer have wifi feature, ``0`` otherwise.
    bool            hasLaminator;               //!< ``1`` if the printer have a laminator, ``0`` otherwise.
    bool            hasMagEnc;                  //!< ``1`` if the printer have mag encoding feature, ``0`` otherwise.
    bool            hasSmartEnc;                //!< ``1`` if the printer have smart encoding feature, ``0`` otherwise.
    bool            hasContactLessEnc;          //!< ``1`` if the printer have contact less feature, ``0`` otherwise.
...
EVOLIS_LIB void evolis_set_barcode(evolis_image_t* img, evolis_barcode_t bc, int height, int showBcText);

/// Set a barcode to the preset image or a blank image.
EVOLIS_LIB int evolis_draw_barcode(evolis_image_t* img, int angle, const char* data);

/// Set the bounding box on the image (used for images or barcode).
...
/// Set the type of ratio to use when scaling the image or barcode.
